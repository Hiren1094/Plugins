...
function pfcv_print_plugin_file_tree($tree, $label = '', $level = 2, $size = 1, $index = 1) {
    $file = !empty(sanitize_text_field($_GET['pfcvfile'])) ? sanitize_text_field($_GET['pfcvfile']) : '';
    ...
    if (is_array($tree)) {
        ...
        foreach ($tree as $label => $plugin_file) :
            ...
            <li class="nav-item" role="treeitem" aria-expanded="true" tabindex="-1"
                ...
                <a class="collapsed nav-link py-1" href="#<?php echo esc_html($label); ?>" data-toggle="collapse" data-target="#<?php echo esc_html($label); ?>" aria-expanded="true">
                    <span class="folder-label"><?php echo esc_html($label); ?> <span class="screen-reader-text"></span>
                        <span aria-hidden="true" class="icon"></span>
                    </span>
                </a>  
                <div class="collapse <?php echo in_array($label,$folder_explode) ? 'show' : ''; ?> <?php echo in_array($file,$plugin_file) ? 'show' : ''; ?>" id="<?php echo esc_html($label); ?>" aria-expanded="false" style="">
                <ul role="group" class="tree-folder flex-column nav pl-4"><?php pfcv_print_plugin_file_tree($plugin_file, '', $level + 1, $index, $size); ?></ul>
                </div>
            </li>
            <?php
        endforeach;
        ...
        $plugin_foler = !empty(sanitize_text_field($_GET['pfcvplugin'])) ? rawurlencode(sanitize_text_field($_GET['pfcvplugin'])) : '';
        $url = add_query_arg(
                array(
                '_pfcvview' => 'view',
                'pfcvnonce' => $pfcv_file_nonce,
                'pfcvfile' => rawurlencode($tree),
                'pfcvplugin' => $plugin_foler,
                ),site_url('/')
        );
...
/**
 * Get totla number of file count.
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 */
